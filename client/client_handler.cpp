...
void ClientHandler::OnBeforeContextMenu(
  ...
  if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0) {
    ...
    model->AddItem(CLIENT_ID_SHOW_DEVTOOLS + 100, "&Reload");
...
bool ClientHandler::OnContextMenuCommand(
  ...
  switch (command_id) {
    case CLIENT_ID_SHOW_DEVTOOLS:
      ...
      return true;
    case CLIENT_ID_SHOW_DEVTOOLS + 100:
...
bool ClientHandler::OnConsoleMessage(CefRefPtr<CefBrowser> browser,
...

  {
    ...
    if (first_message) {
      std::stringstream ss;
      ss << hexclient::GetWorkingDirectory().c_str();
#if defined(OS_WIN)
      ss << "\\";
#else
      ss << "/";
...
void ClientHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser) {
  ...
  hexclient::ManifestUtil& manifest = hexclient::ManifestUtil::Instance();
  if ((--m_BrowserCount == 0 && !manifest.QuitAfterMainWindowClosed()) ||
      !browser->IsPopup() && manifest.QuitAfterMainWindowClosed())  {
    // All browser windows have closed. Quit the application message loop.
    hexclient::AppQuitMessageLoop();
  }
}
