...
def create_readme():
  ...
  node = ''
  if usenode == 1:
    node = '\n* Node.js and libuv core library\n    node.dll\n    uv.dll\n'
...
def generate_msvs_projects(version):
  ...
  gyper = [ 'python', 'tools/gyp_hex', os.path.relpath(os.path.join(output_dir, 'hexclient.gyp'), hex_dir) ]
  RunAction(hex_dir, gyper);
  move_file(os.path.relpath(os.path.join(output_dir, 'hexclient.sln')), \
            os.path.relpath(os.path.join(output_dir, 'hexclient'+version+'.sln')))
...
parser.add_option('--hexclient',
                  dest='hexclient', default=1,
                  help='this is a hexclient')
...
# output directory
if options.client:
  output_dir_base = 'hex_' + hex_ver
else:
  output_dir_base = 'hex_module_' + hex_ver
...
hex_req_paths = eval_file(os.path.join(hex_dir, 'hex_req_paths.gypi'))
hex_req_paths = hex_req_paths['variables']

# read the variables list from the manually edited cef_paths2.gypi file
hex_client_paths = eval_file(os.path.join(hex_dir, 'hex_client_paths.gypi'))
hex_client_paths = hex_client_paths['variables']

if mode == 'standard':
  # create the include directory
  include_dir = os.path.join(output_dir, 'cef/include')
  make_dir(include_dir, options.quiet)

  # create the heX directory
  new_hex_dir = os.path.join(output_dir, 'hex')
  make_dir(new_hex_dir, options.quiet)

  # create the client directory
  client_dir = os.path.join(output_dir, 'client')
  make_dir(client_dir, options.quiet)

  # create the libcef_dll_wrapper directory
  wrapper_dir = os.path.join(output_dir, 'cef/libcef_dll')
  make_dir(wrapper_dir, options.quiet)

  # transfer common include files
  transfer_gypi_files(hex_dir, hex_client_paths['includes_common'], \
                      '../cef/include/', include_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_client_paths['hex_includes_common'], \
                    '../hex/', new_hex_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_client_paths['includes_capi'], \
                      '../cef/include/', include_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_client_paths['includes_wrapper'], \
                      '../cef/include/', include_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_req_paths['autogen_cpp_includes'], \
                      '../cef/include/', include_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_req_paths['hex_autogen_cpp_includes'], \
                    '../hex/', new_hex_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_req_paths['autogen_capi_includes'], \
                      '../cef/include/', include_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_req_paths['hex_autogen_capi_includes'], \
                    '../hex/', new_hex_dir, options.quiet)

  # transfer common cefclient files
  transfer_gypi_files(hex_dir, hex_client_paths['hexclient_sources_common'], \
                      '../hex/client/', client_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_client_paths['hexclient_bundle_resources_common'], \
                      '../hex/client/', client_dir, options.quiet)

  # transfer common libcef_dll_wrapper files
  transfer_gypi_files(hex_dir, hex_client_paths['libcef_dll_wrapper_sources_common'], \
                      '../cef/libcef_dll/', wrapper_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_client_paths['hex_libcef_dll_wrapper_sources_common'], \
                    '../hex/', new_hex_dir, options.quiet)
  transfer_gypi_files(hex_dir, hex_req_paths['autogen_client_side'], \
                      '../cef/libcef_dll/', wrapper_dir, options.quiet)

  # transfer gyp files
  copy_file(os.path.join(script_dir, 'distrib/hexclient.gyp'), output_dir, options.quiet)
  paths_gypi = os.path.join(hex_dir, 'hex_client_paths.gypi')
  data = read_file(paths_gypi)
  data = data.replace('../', '')
  data = data.replace('hex/client/', 'client/')
  write_file(os.path.join(output_dir, 'hex_client_paths.gypi'), data)

  req_paths_gypi = os.path.join(hex_dir, 'hex_req_paths.gypi')
  data = read_file(req_paths_gypi)
  data = data.replace('../', '')
  write_file(os.path.join(output_dir, 'hex_req_paths.gypi'), data)

  # transfer additional files
  transfer_files(hex_dir, script_dir, os.path.join(script_dir, 'distrib/transfer.cfg'), \
                 output_dir, options.quiet)

  # transfer jsoncpp library files
  jsoncpp_dir = os.path.abspath(os.path.join(hex_dir, os.pardir, 'third_party', 'jsoncpp'))
  copy_files(jsoncpp_dir, output_dir, options.quiet)
  jsoncpp_gyp = os.path.join(output_dir, 'jsoncpp', 'jsoncpp.gyp')
  data = read_file(jsoncpp_gyp)
  data = data.replace('\'../../build/shim_headers.gypi\',', '')
  write_file(os.path.join(output_dir, 'jsoncpp', 'jsoncpp.gyp'), data)

if platform == 'windows':
  if options.ninjabuild:
    out_dir = os.path.join(src_dir, 'out')
    libcef_dll_file = 'libcef.dll.lib'
    hex_dll_file = 'hex.dll.lib'
  else:
    out_dir = os.path.join(hex_dir, 'build')
    libcef_dll_file = 'lib/libcef.lib'
    hex_dll_file = 'lib/hex.lib'
  ...
  if mode == 'standard':
    ...
    if not options.allowpartial or path_exists(os.path.join(build_dir, 'hexlient.exe')):
      ...
      copy_file(os.path.join(build_dir, libcef_dll_file), os.path.join(dst_dir, 'libcef.lib'), \
                options.quiet)
      copy_file(os.path.join(build_dir, hex_dll_file), os.path.join(dst_dir, 'hex.lib'), \
                options.quiet)
      ...
      if not options.nosymbols:
        ...
        copy_file(os.path.join(build_dir, 'hex.dll.pdb'), symbol_output_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'node.dll.pdb'), symbol_output_dir, options.quiet)
        if platform == 'windows':
          copy_file(os.path.join(build_dir, 'uv.dll.pdb'), symbol_output_dir, options.quiet)
        else:
          copy_file(os.path.join(build_dir, 'libuv.dll.pdb'), symbol_output_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'v8.dll.pdb'), symbol_output_dir, options.quiet)
    else:
      sys.stderr.write("No Debug build files.\n")

  # transfer Release files
  build_dir = os.path.join(out_dir, 'Release' + build_dir_suffix);
  if not options.allowpartial or path_exists(os.path.join(build_dir, 'hexclient.exe')):
    valid_build_dir = build_dir
  ...
  if mode == 'standard':
    ...
    transfer_gypi_files(hex_dir, hex_client_paths['includes_win'], \
                        '../cef/include/', include_dir, options.quiet)

    # transfer client files
    print '12'
    print client_dir
    transfer_gypi_files(hex_dir, hex_client_paths['hexclient_sources_win'], \
                        '../hex/client/', client_dir, options.quiet)
